...
module VizborCLI::Mongo
  ...
  # Default options.
  raw_options : RawMongoDriverOptions = {
  ...
  def create_options_yaml : Nil
    ...
    unless Dir.exists?(path)
      Dir.mkdir_p(path)
      File.open(path / "options.yml", "w") do |file|
        comments = "# Mongo Driver Options\n" \
                   "# For fields of type Time::Span use example: 10.seconds\n" \
                   "# For reference:\n" \
                   "# https://elbywan.github.io/cryomongo/Mongo/Client.html\n" \
                   "# https://elbywan.github.io/cryomongo/Mongo/Options.html"
        yaml = raw_options.to_yaml
        file.print "#{comments}\n#{yaml}"
      end
    end
  end

  def mongo_driver_options : Mongo::Options
  ...
  def to_span(value : String) : Time::Span
    arr = value.split(".")
  ...
  def arr_to_span(arr : Array(String)) : Time::Span?
    case arr[1]
